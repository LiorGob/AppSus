...
// windows.theBook = books;

function getGoogleBooks(title) {
  // var books=[]
...
function getEmpty() {
  const review = { id: makeId(), name: '', rate: 1, date: '', txt: '' };
  return review;
}
...
// function getByIdx(itemIdx) {
//   const bookidx = books.findIndex(book => book.id === itemIdx.id)
// }
...
function addBook(book) {
  ...
  newBook.listPrice = {}
  newBook.id = book.id
  newBook.thumbnail = book.volumeInfo.imageLinks ? book.volumeInfo.imageLinks.thumbnail : ''
  newBook.title = book.volumeInfo ? book.volumeInfo.title : ''
  newBook.pageCount = book.pageCount ? book.pageCount : ''
  newBook.publishedDate = book.volumeInfo ? new Date(book.volumeInfo.publishedDate).getFullYear() : ''
  newBook.listPrice.amount = book.saleInfo.retailPrice ? book.saleInfo.retailPrice.amount : ''
  newBook.listPrice.currencyCode = book.saleInfo.retailPrice ? book.saleInfo.retailPrice.currencyCode : ''
  newBook.listPrice.isOnSale = book.saleInfo.saleability ? book.saleInfo.saleability : ''
  newBook.description = book.volumeInfo.description ? book.volumeInfo.description : ''
  books = [newBook, ...books]
  // storageService.saveToStorage
  return Promise.resolve('add book')
}
...
//   getBookById(bookId)
//       .then(book => {
//           getReviewById(book, reviewId)
//               .then(reviewIdx => book.splice(reviewIdx, 1))
//       })
// }


// bookToSave.review.push(review);
